export default {
  async fetch(request, env) {
    ...
    // Handle your existing R2 asset serving
  ...
  const prefetchScript = `
    <script>
      ...
      function prefetchVideoSegments(videoUrls) {
        // Prefetch first 10 seconds of each critical video
        videoUrls.forEach(function(videoUrl) {
          fetch(videoUrl, {
            headers: { 'Range': 'bytes=0-1048576' } // First 1MB (~10 seconds)
...
async function getAllMediaUrls(env) {
  ...
  const listResponse = await env.ASSETS.list({
    prefix: 'public/' // Adjust prefix as needed
  });
  ...
  // Sort by likely importance (you can customize this logic)
  images.sort((a, b) => {
    // Prioritize smaller images first
  ...
  videos.sort((a, b) => {
    // Prioritize hero/featured videos
...
async function cacheMediaBatch(origin, mediaPaths, cache, type) {
  const batchSize = type === 'image' ? 8 : 2; // Smaller batches for videos
...
async function cacheVideoSegments(origin, videoPaths, cache) {
  // Cache first 1MB of each video for instant playback start
...
// Enhanced video prefetch with adaptive bitrate
function adaptiveVideoPrefetch() {
  // Check connection speed
  const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
  const isSlowConnection = connection && (connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g');
  
  if (isSlowConnection) {
    // Only prefetch video thumbnails on slow connections
    prefetchVideoThumbnails();
  } else {
    // Prefetch full video segments on fast connections
    prefetchVideoSegments();
  }
}
